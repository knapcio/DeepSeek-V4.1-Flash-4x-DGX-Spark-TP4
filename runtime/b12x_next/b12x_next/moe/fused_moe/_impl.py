...
from b12x_next._lib.quant.block_codec import BLOCK_CODECS
...
from b12x_next._lib.compile_plan import launch_triton, planning
from b12x_next._lib.program_cache import register_program_cache
from b12x_next._lib.compiler import (
    KernelCompileSpec,
    compile as b12x_compile,
)
from b12x_next._lib.intrinsics import (
    align_up,
    as_grouped_scale_view,
    swizzle_block_scale,
)
from b12x_next._lib.utils import (
    current_cuda_stream,
...
from b12x_next.moe._shared.routing import (
    route_topk as triton_route_topk,
    _validate_score_options,
)
from b12x_next.moe._shared.kernels.relu2 import (
    MoEDynamicKernelRelu2,
    MoEMicroKernelRelu2,
)
from b12x_next.moe._shared.kernels.silu import (
    MoEDynamicKernelSilu,
    MoEDynamicKernelSwiGLUOAI,
    MoEMicroKernelSilu,
    MoEMicroKernelSwiGLUOAI,
)
from b12x_next.moe._shared.kernels.situ import (
    MoEDynamicKernelSitu,
    MoEMicroKernelSitu,
)
from b12x_next.moe._shared.kernels.activations import (
    SITU,
...
from b12x_next.moe._shared.trellis_codebooks import LUT_E4M3
from b12x_next.preparation import DeviceIdentity
from b12x_next._lib.runtime_control import (
    raise_if_kernel_resolution_frozen,
)
from b12x_next._lib.scratch import (
    ScratchBufferSpec,
    scratch_buffer_spec,
    scratch_tensor,
)
...
_B12X_TIMING = (
    os.getenv("B12X_NEXT_TIMING", "0") == "1" or os.getenv("VLLM_B12X_TIMING", "0") == "1"
)
_B12X_TIMING_THRESHOLD_MS = float(
    os.getenv(
        "B12X_NEXT_TIMING_THRESHOLD_MS",
...
_DYNAMIC_WORK_SOURCE_ENV = "B12X_NEXT_DYNAMIC_WORK_SOURCE"
_DYNAMIC_WORK_SOURCE_DEFAULT = "materialized_queue"
_DYNAMIC_EXTERNAL_ROUTE_PLAN_ENV = "B12X_NEXT_DYNAMIC_EXTERNAL_ROUTE_PLAN"
...
_DYNAMIC_W4A8_REPACKED_ENV = "B12X_NEXT_DYNAMIC_W4A8_REPACKED"
_DYNAMIC_W4A8_SHARE_INPUT_ENV = "B12X_NEXT_DYNAMIC_W4A8_SHARE_INPUT"
_DYNAMIC_W4A8_MATERIALIZED_ENV = "B12X_NEXT_DYNAMIC_W4A8_MATERIALIZED"
# Captured by plan construction as a constraint on NVFP4 materialization.
_DYNAMIC_NVFP4_MATERIALIZED_ENV = "B12X_NEXT_NVFP4_DYNAMIC_MATERIALIZED"
_W4A8_CONVERT_SCRATCH_MB_ENV = "B12X_NEXT_W4A8_CONVERT_SCRATCH_MB"
_W4A8_CONVERT_SCRATCH_MB_DEFAULT = 64
# The source-format vocabulary is owned by b12x_next.moe._shared.execution;
...
def _dynamic_deterministic_output_enabled(
    *, quant_mode: str, device: torch.device
    ...
    if _normalize_quant_mode(quant_mode) == "w4a16":
        return False
    return _env_flag("B12X_NEXT_DYNAMIC_DETERMINISTIC_OUTPUT", default=False)
...
def _get_activation_kernel_spec(
    ...
    *,
    ...
    if _normalize_quant_mode(quant_mode) == "w4a16":
        raise ValueError(
            "W4A16 dispatch uses b12x_next.moe._shared.kernels.w4a16.kernel directly"
        )
    if (
...
# B12X_NEXT_DYNAMIC_TILE_MN="64x128" or programmatically (tp_moe._DYNAMIC_TILE_MN_OVERRIDE).
...
def _dynamic_tile_mn_override() -> Tuple[int, int] | None:
    env = os.environ.get("B12X_NEXT_DYNAMIC_TILE_MN")
    if env:
...
def _select_dynamic_tile_mn(
    ...
    *,
    ...
    B12X_NEXT_DYNAMIC_TILE_MN / programmatic override wins (for benchmarking).
    """
    quant_mode = _normalize_quant_mode(quant_mode)
    ovr = _dynamic_tile_mn_override()
    if ovr is not None:
...
_DYNAMIC_SWAP_AB_OVERRIDE = os.environ.get("B12X_NEXT_DYNAMIC_SWAP_AB")
...
def clear_tp_moe_caches() -> None:
    ...
    """
    from b12x_next.moe._shared.kernels.w4a16.kernel import (
        clear_w4a16_kernel_cache,
    )
    from b12x_next.moe._shared.kernels.w4a8_compact_micro import (
...
_FAST_MATH_DEFAULT = _env_flag("B12X_NEXT_FAST_MATH", default=True)
...
def _get_micro_dynamic_cutover_pairs(quant_mode: str = "nvfp4") -> int:
    ...
    if cached is None:
        cutover = os.environ.get("B12X_NEXT_MICRO_DYNAMIC_CUTOVER_PAIRS")
        if cutover is None:
...
def _arena_core_token_counts(
    *,
    ...
    if quant_mode == "w4a16" and bucket_w4a16_tokens:
        from b12x_next.moe._shared.kernels.w4a16.host import (
...
def _dynamic_multicta_enabled() -> bool:
    ...
    if _DYNAMIC_MULTICTA_CACHE is None:
        multicta_env = _first_env(
            "B12X_NEXT_DYNAMIC_ENABLE_MULTICTA",
            "B12X_NEXT_LEVEL10_ENABLE_MULTICTA",
        )
...
def _dynamic_down_scale_enabled() -> bool:
    ...
    if _DYNAMIC_DOWN_SCALE_CACHE is None:
        _DYNAMIC_DOWN_SCALE_CACHE = _env_flag(
            "B12X_NEXT_ENABLE_DYNAMIC_DOWN_SCALE", default=False
        )
...
def _derive_w4a8_weight_grids(
    ...
    from b12x_next.moe._shared.kernels.reference import (
...
def _prepare_w6a8_from_mxfp6_source(
    *,
    ...
    scales.  Returns a
    :class:`b12x_next.moe._shared.kernels.w6a8.PreparedW6A8MXFP6Weights`.
    """
    from b12x_next.moe._shared.kernels.w6a8 import prepare_w6a8_mxfp6_weights
...
def prepare_b12x_fp4_moe_weights(
    *,
    ...
    if plan.source_format == "exl3":
        from b12x_next.moe._shared.kernels.w4a16.exl3 import (
            Exl3Layer,
    ...
    if WeightPreparationTransform.W4A16_NATIVE in plan.transforms:
        ...
        from b12x_next.moe._shared.kernels.w4a16.prepare import (
...
def prepare_b12x_iq2_xs_weights(*, plan, weights) -> B12XFP4ExpertWeights:
    ...
    from b12x_next.moe._shared.kernels.w4a16.iq2_xs import prepare_iq2_xs_moe_weights
...
def prepare_b12x_trellis_v2_weights(
    *,
    ...
    from b12x_next.moe.fused_moe.config import TrellisConfig
    from b12x_next.moe.fused_moe.trellis import prepare_trellis_weights
    from b12x_next.moe.fused_moe.weights import TrellisWeights
...
def prewarm_w4a16_fc2_e8m0(experts, *, route_ids_dtype: torch.dtype):
    ...
    from b12x_next.moe._shared.kernels.w4a16 import kernel as w4a16_kernel
    from b12x_next.moe._shared.kernels.w4a16.prepare import W4A16FC2Weights
...
def run_w4a16_fc2_e8m0(
    ...
    *,
    ...
    from b12x_next.moe._shared.kernels.w4a16 import kernel as _w4a16_kernel  # noqa: F401
    from b12x_next.moe._shared.kernels.w4a16.prepare import W4A16FC2Weights
    ...
    if launcher is None:
        torch.ops.b12x_next.w4a16_fc2_direct_launch(
...
def _plan_projection_mixed_trellis_launches(
    *,
    ...
    from b12x_next.moe._shared.kernels.w4a16.host import max_packed_route_slots
    from b12x_next.moe._shared.kernels.w4a16.kernel import _DEFAULT_MAX_SHARED_MEM
    from b12x_next.moe._shared.kernels.w4a16.mixed_trellis import (
        MixedTrellisBuffers,
...
def _bind_projection_mixed_trellis_from_views(
    *,
    ...
    from b12x_next.moe.fused_moe.trellis import PreparedProjectionTrellisWeights
...
def _resolve_trellis_route_block_size(caps: TPMoEScratchCaps) -> int | None:
    ...
    if _normalize_quant_mode(caps.quant_mode) != "w4a16":
        ...
        return None
    from b12x_next.moe._shared.kernels.w4a16.host import select_route_block_size_m
...
def _get_impl_mac(impl: str, *, routed_rows: int | None = None) -> int:
    ...
    override_name = f"B12X_NEXT_{impl.upper()}_MAX_ACTIVE_CLUSTERS"
    if impl.startswith("dynamic"):
        mac_override = _first_env(
            override_name,
            "B12X_NEXT_DYNAMIC_MAX_ACTIVE_CLUSTERS",
            "B12X_NEXT_LEVEL10_MAX_ACTIVE_CLUSTERS",
        )
    else:
...
def _select_micro_mma_tiler_mn(
    ...
    *,
    ...
    if os.environ.get("B12X_NEXT_MOE_TILE_MN"):
        return tuple(int(x) for x in os.environ["B12X_NEXT_MOE_TILE_MN"].split("x"))
    sm_count = get_num_sm(torch.device("cuda"))
    coarse_tile = (128, 128)
...
def _get_micro_kernel(
    ...
    *,
    ...
    reuse_compiled = not planning() and os.environ.get("B12X_NEXT_MICRO_REUSE_COMPILED", "1") != "0"
    if reuse_compiled:
    ...
    compile_kwargs = {}
    compile_options = os.environ.get("B12X_NEXT_DIRECT_CUTE_OPTIONS", "")
    if compile_options:
...
def _launch_dynamic_flat(
    *,
    ...
    if (
        ...
        and _first_env(
            f"B12X_NEXT_{mac_backend.upper()}_MAX_ACTIVE_CLUSTERS",
            "B12X_NEXT_DYNAMIC_MAX_ACTIVE_CLUSTERS",
            "B12X_NEXT_LEVEL10_MAX_ACTIVE_CLUSTERS",
        )
        is None
...
@torch.library.custom_op(
    "b12x_next::tp_moe_dynamic_launch",
...
def _launch_dynamic(
    *,
    ...
    torch.ops.b12x_next.tp_moe_dynamic_launch(
...
def _launch_dynamic_topk_sum(
    *,
    ...
    torch.ops.b12x_next.w4a16_topk_sum_launch(
        route_output,
        output,
        m,
        num_topk,
...
def _launch_compact_micro_flat(
    *,
    ...
    use_native_nvfp4_split = (
        ...
        and 1 <= m <= 8
        and activation == "silu"
        and os.environ.get("B12X_NEXT_NVFP4_SPLIT_DECODE", "0") != "0"
    )
...
def _tiny_decode_enabled() -> bool:
    # Default on; B12X_NEXT_W4A8_TINY_DECODE=0 is the kill switch.
    return os.environ.get("B12X_NEXT_W4A8_TINY_DECODE", "1") != "0"
...
def _get_tiny_decode_kernel(
    ...
    *,
    ...
    from b12x_next.moe._shared.kernels.tiny_decode import (
...
@torch.library.custom_op(
    "b12x_next::tp_moe_tiny_decode_launch",
...
@torch.library.custom_op(
    "b12x_next::tp_moe_compact_micro_launch",
...
def _launch_micro(
    *,
    ...
    if quant_mode == "w4a8_mx" and not w4a8_trellis:
        ...
        torch.ops.b12x_next.tp_moe_tiny_decode_launch(
            workspace.barrier_count,
            workspace.barrier_epoch,
    ...
    if flat_ids.dtype == torch.int32 and flat_ids.is_contiguous():
        ...
        launch_ids = workspace.compact_topk_ids[: flat_ids.numel()]
        launch_ids.copy_(flat_ids.to(torch.int32))
    torch.ops.b12x_next.tp_moe_compact_micro_launch(
        workspace.barrier_count,
        workspace.barrier_epoch,
        workspace.micro_intermediate,
        weights.w1_storage,
...
def b12x_moe_fp4(*, binding: TPMoEFP4Binding) -> torch.Tensor:
    ...
    if compact_w4a8_micro:
        ...
        from b12x_next.moe._shared.kernels.w4a16.kernel import _w4a16_topk_sum_launch_flat
    ...
    if impl == "dynamic":
        ...
        _launch_micro(
            ...
            share_input_across_experts=(
                ...
                and os.environ.get("B12X_NEXT_MICRO_SHARE_INPUT_ACROSS_EXPERTS", "1") != "0"
            ),
            share_expert_scales=(
                activation in ("relu2", "silu")
                and a1_gscale.numel() == 1
