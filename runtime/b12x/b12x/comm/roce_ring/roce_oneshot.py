...
from ._path_config import (
    ...
    validate_four_path_tp4_mapping as _validate_four_path_tp4_mapping,
)
...
DEFAULT_THREADS = 512
DEFAULT_BLOCKS = 8
ADAPTIVE_GEOMETRY = ("weighted8-capture-only-v1", ((16384, 2), (65536, 4)), 8)


def _allreduce_blocks(nbytes: int) -> int:
    """Select a divisor-of-eight grid; each operation contributes eight arrivals."""
    return 2 if nbytes <= 16384 else 4 if nbytes <= 65536 else 8
    ...
    def __init__(
        ...
        *,
        ...
        if self.world_size != 4 or int(threads) != 512 or int(blocks) != 8:
            raise ValueError("adaptive weighted8 requires world_size=4, threads=512, base blocks=8")
        ...
        if path_failures:
            raise RuntimeError(
                "RoCE all-reduce path preflight failed: " + "; ".join(path_failures)
            )
        ...
        config = {
            ...
            # Reuse an existing compared key so an unmodified rank-zero peer
            # also rejects this adaptive package before connecting the QPs.
            "blocks": (self._blocks, ADAPTIVE_GEOMETRY),
        }
        ...
        with self._lock:
            ...
            with torch.cuda.device(self.device), context:
                ...
                launcher(
                    ...
                    self._epoch_address,
                    self.spin_limit,
                    _allreduce_blocks(nbytes) if capturing else self._blocks,
                )
                if dst is not out:
                    out.copy_(dst)
