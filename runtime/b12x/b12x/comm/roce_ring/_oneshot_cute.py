...
Each all-reduce uses 2, 4, or 8 CTAs and increments both counters by 8/grid
per arriving CTA. Every successful operation contributes eight to each counter,
so arbitrary grid transitions and the unchanged eight-CTA all-gather preserve
the modulo-eight boundary. Message size is a runtime scalar.
"""
...
class _RoceOneshotLaunch:
    ...
    def kernel(
        ...
        arrival_step = Uint32(8) // Uint32(gdim)
        ...
        if poisoned == Uint32(0):
            ...
            if Int32(tidx) == Int32(0):
                prior = atomic_add_relaxed_gpu_u32(stage_counter_ptr, arrival_step)
                if (prior + arrival_step) % Uint32(8) == Uint32(0):
            ...
            cute.arch.sync_threads()
            if Int32(tidx) == Int32(0):
                prior = atomic_add_relaxed_gpu_u32(tail_counter_ptr, arrival_step)
                if (prior + arrival_step) % Uint32(8) == Uint32(0):
                    fence_sc_gpu()
...
def get_launcher(
    ...
    raw = b12x_compile(
        ...
        1,
        current_cuda_stream(),
        compile_spec=KernelCompileSpec.from_key("comm.roce_ring.oneshot", 2, cache_key),
    )
