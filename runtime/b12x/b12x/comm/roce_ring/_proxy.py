...
def _cache_dir() -> Path:
    ...
    return Path(root) / "b12x" / "roce_ring"
...
def _compiler() -> str:
    ...
    for candidate in (os.getenv("CC"), "gcc", "cc", "clang"):
        if candidate and shutil.which(candidate):
            return candidate
    raise RuntimeError(
        "b12x.comm.roce_ring needs a C compiler (gcc/cc/clang) and libibverbs headers "
...
class Proxy:
    ...
    def path_counters(self) -> list[dict[str, int | str | None]]:
        ...
        for peer in range(self.world_size):
            ...
            path_count = (
                self.opposite_paths
                if self.world_size == 4 and (peer - self.rank) % self.world_size == 2
                else 2
            )
